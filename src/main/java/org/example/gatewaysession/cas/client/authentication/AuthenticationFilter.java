...
import org.example.gatewaysession.cas.client.configuration.ConfigurationKeys;
import org.example.gatewaysession.cas.client.util.AbstractCasFilter;
import org.example.gatewaysession.cas.client.util.CommonUtils;
import org.example.gatewaysession.cas.client.util.ReflectUtils;
import org.example.gatewaysession.cas.client.validation.Assertion;
...
public class AuthenticationFilter extends AbstractCasFilter {
    ...
    public final void doFilter(final ServletRequest servletRequest, final ServletResponse servletResponse,
        ...
        // TODO: Gateway没有session，这段代码不需要
        final HttpSession session = request.getSession(false);
        final Assertion assertion = session != null ? (Assertion) session.getAttribute(CONST_CAS_ASSERTION) : null; // 从session中获取名为“const_cas_assertion”的assertion对象

        if (assertion != null) {
            filterChain.doFilter(request, response); // 这里跟gateway的传递到下一个过滤器不同写法
            return;
        }

        final String serviceUrl = constructServiceUrl(request, response);   //生成serviceUrl
        final String ticket = retrieveTicketFromRequest(request);   //从request中获取参数ticket
        final boolean wasGatewayed = this.gateway && this.gatewayStorage.hasGatewayedAlready(request, serviceUrl); // 从session中获取 _const_cas_gateway_ 的value值

        if (CommonUtils.isNotBlank(ticket) || wasGatewayed) {   //如果ticket不为空，本过滤器处理完成，处理下个过滤器
            filterChain.doFilter(request, response);
            return;
        }
        ...
        final String urlToRedirectTo = CommonUtils.constructRedirectUrl(this.casServerLoginUrl,
            getProtocol().getServiceParameterName(), modifiedServiceUrl, this.renew, this.gateway, this.method); //生成重定向URL

        logger.debug("redirecting to \"{}\"", urlToRedirectTo);
        this.authenticationRedirectStrategy.redirect(request, response, urlToRedirectTo); // 跳转到CAS服务器的登录页面
    }
