...
public abstract class AbstractTicketValidationFilter extends AbstractCasFilter {
    ...
    public final void doFilter(final ServletRequest servletRequest, final ServletResponse servletResponse,
        ...
        final String ticket = retrieveTicketFromRequest(request);  //从request中获取ticket

        if (CommonUtils.isNotBlank(ticket)) {  //ticket不为空，验证ticket，否则本过滤器处理完成，处理下个过滤器
            logger.debug("Attempting to validate ticket: {}", ticket);

            try {
                final Assertion assertion = this.ticketValidator.validate(ticket,
                        constructServiceUrl(request, response));  //验证ticket并产生Assertion对象，错误抛出TicketValidationException异常

                logger.debug("Successfully authenticated user: {}", assertion.getPrincipal().getName());

                request.setAttribute(CONST_CAS_ASSERTION, assertion); //request设置assertion

                if (this.useSession) { // TODO: 不用session
                    request.getSession().setAttribute(CONST_CAS_ASSERTION, assertion);
                }
                onSuccessfulValidation(request, response, assertion); // 成功回调

                if (this.redirectAfterValidation) { // 表示是否验证通过后重新跳转到该URL，但是不带参数ticket
                ...
                onFailedValidation(request, response);  // 失败回调
        ...
        filterChain.doFilter(request, response); //本过滤器处理完成，处理下个过滤器
